...
class XenLiteXen3(nn.Module):
    def __init__(self, in_channels: int = 3, model_channels: int = 64, num_classes: int = 10):
        super().__init__()
        ...
        # 96x96 → 48x48 → 24x24 → 12x12 → 6x6 → 3x3 (5단계 다운샘플링)
        self.down1 = DownBlock(model_channels, model_channels, time_emb_dim)           # 96x96 → 48x48
        self.down2 = DownBlock(model_channels, model_channels * 2, time_emb_dim)      # 48x48 → 24x24
        self.down3 = DownBlock(model_channels * 2, model_channels * 3, time_emb_dim, has_attn=True)  # 24x24 → 12x12
        self.down4 = DownBlock(model_channels * 3, model_channels * 4, time_emb_dim)  # 12x12 → 6x6
        self.down5 = DownBlock(model_channels * 4, model_channels * 5, time_emb_dim)  # 6x6 → 3x3
        
        # 중간 처리 블록
        self.middle = nn.Sequential(
            ResidualBlock(model_channels * 5, model_channels * 5, time_emb_dim),
            AttentionBlock(model_channels * 5),
            ResidualBlock(model_channels * 5, model_channels * 5, time_emb_dim)
        )
        
        # 업샘플링 블록 (5단계)
        self.up1 = UpBlock(model_channels * 5, model_channels * 4, time_emb_dim)      # 3x3 → 6x6
        self.up2 = UpBlock(model_channels * 4, model_channels * 3, time_emb_dim)      # 6x6 → 12x12
        self.up3 = UpBlock(model_channels * 3, model_channels * 2, time_emb_dim, has_attn=True)  # 12x12 → 24x24
        self.up4 = UpBlock(model_channels * 2, model_channels, time_emb_dim)          # 24x24 → 48x48
        self.up5 = UpBlock(model_channels, model_channels, time_emb_dim)              # 48x48 → 96x96
        
        norm_groups = min(8, model_channels)
        if model_channels % norm_groups != 0:
            norm_groups = model_channels // ((model_channels // 8) + 1)
    ...
    def forward(self, x, timesteps, class_labels=None):
        ...
        # 다운샘플링 (스킵 연결 저장)
        x, skip1 = self.down1(x, time_emb)  # 64 → 64, skip1: 64 channels
        x, skip2 = self.down2(x, time_emb)  # 64 → 128, skip2: 128 channels
        x, skip3 = self.down3(x, time_emb)  # 128 → 192, skip3: 192 channels
        x, skip4 = self.down4(x, time_emb)  # 192 → 256, skip4: 256 channels
        x, skip5 = self.down5(x, time_emb)  # 256 → 320, skip5: 320 channels
        
        # 중간 처리
        x = self.middle[0](x, time_emb)  # 320 → 320
        x = self.middle[1](x)            # 320 → 320
        x = self.middle[2](x, time_emb)  # 320 → 320
        
        # 업샘플링 (스킵 연결과 결합)
        x = self.up1(x, skip5, time_emb)  # 320 + 320 → 256
        x = self.up2(x, skip4, time_emb)  # 256 + 256 → 192
        x = self.up3(x, skip3, time_emb)  # 192 + 192 → 128
        x = self.up4(x, skip2, time_emb)  # 128 + 128 → 64
        x = self.up5(x, skip1, time_emb)  # 64 + 64 → 64
